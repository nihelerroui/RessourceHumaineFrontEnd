...
import {
  ...
  selectPaysList,
  selectPaysLoading,
} from "src/app/store/pays/pays.selectors";
import {
  addPays,
  deletePays,
  loadPays,
  updatePays,
} from "src/app/store/pays/pays.actions";
...
export class PaysListComponent implements OnInit {
  ...
  filterPays() {
    this.store.select(selectPaysList).subscribe((pays) => {
      this.filteredPaysList = pays.filter(
        (p) =>
  ...
  pageChanged(event: any) {
    ...
    this.paginatedPaysList = this.filteredPaysList.slice(
      startIndex,
